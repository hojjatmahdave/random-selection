...
const App = () => {
  ...
  return (
    <div className="relative flex flex-col items-center  bg-gradient-to-tr pt-20 pl-32 pr-32 pb-5 text-white from-violet-600 to-indigo-600 h-screen w-full">
      <header>
        <h1 className="font-mono text-5xl">Random Selector</h1>
      </header>
      <main className="grid grid-cols-4 gap-2 my-10">
        {Data.map((item, index) => {
          return <Card index={index} item={item} key={index} Filter={Filter} />;
        })}
      </main>
      <button
        onClick={randomNumber}
        type="button"
        className="mt-auto group  relative inline-block overflow-hidden text-lg  border-gray-100 w-52 h-16  font-medium  hover:text-cyan-400 focus:outline-none   font-mono bg-black outline-none text-white rounded shadow-sm hover:shadow-lg  hover:scale-105 transition-all duration-200 "
      >
